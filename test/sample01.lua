...
local function alertSample(samples, n)
   for i,sample in ipairs(samples) do
      print(string.format("sample %d: tick=%d level=0x%08X", i, sample.tick, sample.level))
   end
end
...
gpio.setGetSamplesFunc(alertSample, bit32.lshift(1, pinp) + bit32.lshift(1, pout))
